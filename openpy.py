...
def display():
    ...
    wb = op.load_workbook("excelDb.xlsx")
    sheet = wb.active
...
def input_validation():
    ...
    if not birth.isdigit():
        messagebox.showerror("Error","Birthyear must be number!")
        return False
    return True
...
def saving():
    ...
    sheet.append([new_id, first, middle, last,by,age])
...
def updt():
    ...
    for rows in sheet.iter_rows(min_row = 2):
        if str(rows[0].value) == str(rec_id):
            rows[1].value = last
            rows[2].value = first
            rows[3].value = middle
            rows[4].value = birth
...
def delt():
    ...
    for i, row in enumerate(sheet.iter_rows(min_row=2), start = 2):
        if str(row[0].value) == str(rec_id):
            sheet.delete_rows(i)
            break

    workbook.save("excelDb.xlsx")
    messagebox.showinfo("Successfu;", "Record deleted successfully.")          
...
delete_btn = tk.Button(window, text="Delete",  bg="red", fg="white", command = delt)
...
table.bind("<<TreeviewSelect>>", auto_pup)
